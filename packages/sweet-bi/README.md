...
`sweet-etl` wraps `boti_data`/`boti_dask`), but ClickHouse is a plausible
...
```python
from sweet_bi import get_clickhouse_settings

settings = get_clickhouse_settings()  # host, port, database, user, password, secure, verify
```

Loaded the same way `sweet.settings.get_settings()` is: YAML defaults
from `{SWEET_CONFIG_DIR}/clickhouse.yaml`, overridden by `CLICKHOUSE_*`
in `{SWEET_CONFIG_DIR}/.env`, overridden by the process environment.

It also registers into the `sweet.packages` entry-point group, so the
...
runtime dependency exists, follow `sweet-etl`'s shape: depend on it
