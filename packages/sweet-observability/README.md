...
```python
from sweet_observability import get_logger, get_observability_settings
...
from `{SWEET_CONFIG_DIR}/observability.yaml`, overridden by `OO_*` in
`{SWEET_CONFIG_DIR}/.env`, overridden by the process environment.
