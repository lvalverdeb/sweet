...
Generic ETL entry point for the `sweet` suite. This package does **not**
...
sessions from environment configuration. `sweet-etl` is a thin facade
that re-exports both and wires a pipeline run to a managed session:

```python
from sweet_etl import DataHelper, run_with_dask_session, SinkPipeline
...
`sweet-etl` dependency, same as the `sql()` connectivity check).
...
Optional add-on to the `sweet` suite — install with:
